...
class FireFlowEditFluxPipeline(RFEditingFluxPipeline):
    def denoise(
        self,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for step_idx, (t_curr, t_prev) in enumerate(zip(timesteps[:-1], timesteps[1:], strict=True)):
                ...
                if not with_second_order:
                    ...
                    latents = latents + (sigma_prev - sigma_curr) * mid_noise_pred
                    latents = latents.to(dtype)
    ...
    def reconstruction(
        ...
        if change_to_krea:
            ...
            print("Changing to Krea model for reconstruction...")
            self.transformer.to('cpu')
            del self.transformer
            self.transformer = FluxTransformer2DModel.from_pretrained('black-forest-labs/FLUX.1-Krea-dev', torch_dtype=torch.bfloat16, subfolder='transformer').to(device)
