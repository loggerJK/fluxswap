...
class Model(nn.Module):
    ...
    def forward(self, x_in):
        feature = self.base_model(x_in["face"]) # (B, C, H, W), (B, 32, 7, 7)
        batch_size = feature.size(0)
        feature = feature.flatten(2)
        feature = feature.permute(2, 0, 1) # (B, C, HW) -> (HW, B, C)
        ...
        feature = feature.permute(1, 2, 0) # (HW+1, B, C) -> (B, C, HW+1)

        feature = feature[:,:,0]  # get cls token, shape: (B, C)

        gaze = self.feed(feature)
        
        return gaze
    
    def forward_feature(self, x_in):
        feature = self.base_model(x_in["face"]) # (B, C, H, W), (B, 32, 7, 7)
        batch_size = feature.size(0)
        feature = feature.flatten(2)
        feature = feature.permute(2, 0, 1) # (B, C, HW) -> (HW, B, C)
