...
def main(args):
    ...
    if rank == 0:
        print(f"[INFO] Using device: {device} x {world_size} GPUs")
    ...
    src_img_path_list = src_img_path_list[rank::world_size] # Split data for multi-gpu
    ...
    for src_img_path in tqdm(src_img_path_list, desc=f'GPU {rank} Processing Image'):
        ...
        if not os.path.exists(cond_img_path):
            cond_img_path = os.path.join(trg_img_path_base, cond_dir, f"{src_num}.jpg")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--condition_type", type=str, default=None, help="Condition type", choices=['blur_landmark', 'blur_landmark_iris'])
